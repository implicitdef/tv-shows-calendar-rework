...
app.get("/shows", async (req, res, next) => {
  try {
    ...
    const seriesFiltered = req.query.q && typeof req.query.q === "string" ?
      series.filter((serie) =>
...
app.get("/shows/default", async (req, res, next) => {
  try {
    const data = await DbService.loadData();
    const series = data.map((serieAndSeasons) => serieAndSeasons.serie);
    const seriesFiltered = series.filter((serie) => Conf.defaultShowsIds.includes(serie.id));
    res.json(seriesFiltered);
  } catch (err) { next(err); }
});
...
app.get("/shows/:showId/seasons", async (req, res, next) => {
  try {
    ...
    const data = await DbService.loadData();
    const serieAndSeason = data.find((serieAndSeasons) =>
      String(serieAndSeasons.serie.id) === showId,
    );
    if (!serieAndSeason) {
...
app.post("/me/shows/:serieId", Auth.middleware, async (req, res, next) => {
  try {
    const liReq = req as Auth.LoggedInRequest;
    await DbService.addSerieToUser(liReq.userId, parseInt(req.params.serieId, 10));
...
app.delete("/me/shows/:serieId", Auth.middleware, async (req, res, next) => {
  try {
    const liReq = req as Auth.LoggedInRequest;
    await DbService.removeSerieFromUser(liReq.userId, parseInt(req.params.serieId, 10));
    res.json({message: "Done"});
  } catch (err) { next(err); }
});
